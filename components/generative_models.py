...
class ActionStateTracking(CoreModel):
  ...
  def generate(self, full_history):
    dst_outputs = self.encoder.generate(**full_history, eos_token_id=self.tokenizer.eos_token_id)
    return self.tokenizer.batch_decode(dst_outputs, skip_special_tokens=True)
